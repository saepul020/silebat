...
class BaseMasterDataForm(forms.ModelForm):
    ...
    def _set_placeholder_choice(self, field_name, placeholder_text=None):
        ...
        placeholder_text = placeholder_text or f"Pilih {(field.label or field_name).strip().lower()}"
...
class AssetBaseForm(BaseMasterDataForm):
    ketersediaan_info = forms.CharField(label='Ketersediaan', required=False, disabled=True)
    tanggal_pemeliharaan_info = forms.DateField(label='Tanggal Pemeliharaan', required=False, disabled=True)
    tanggal_perbaikan_info = forms.DateField(label='Tanggal Perbaikan', required=False, disabled=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['foto_barang'].widget = forms.FileInput(attrs={'accept': '.jpg,.jpeg,.png', 'class': 'input-file--proxy', 'data-inline-file-input': 'true', 'data-inline-file-placeholder': 'Pilih file', 'data-inline-file-extensions': 'jpg,jpeg,png', 'data-inline-file-error': 'Foto Barang hanya boleh berupa file JPG, JPEG, atau PNG.'})
        ...
        self._set_input_attrs({
            'nama_barang': {'placeholder': 'Masukkan nama barang', 'autocomplete': 'off', 'data-status-dependent': 'true'},
            'tipe_merek_barang': {'placeholder': 'Masukkan tipe / merek barang', 'autocomplete': 'off', 'data-status-dependent': 'true'},
            'jenis_barang': {'placeholder': 'Masukkan jenis barang', 'autocomplete': 'off', 'data-status-dependent': 'true'},
            'kode_aset_bmn': {'placeholder': 'Masukkan kode aset BMN', 'autocomplete': 'off', 'data-bmn-code-field': 'true', 'data-status-dependent': 'true'},
            'kode_laboratorium': {'placeholder': 'Masukkan kode laboratorium', 'autocomplete': 'off', 'data-status-dependent': 'true'},
            'volume': {'min': '0', 'data-status-dependent': 'true'},
            'satuan': {'data-status-dependent': 'true'},
            'tahun_perolehan': {'placeholder': 'Contoh: 2024', 'min': '1900', 'data-status-dependent': 'true'},
            'kondisi_barang': {'data-status-dependent': 'true'},
            'lokasi_barang': {'placeholder': 'Masukkan lokasi barang', 'autocomplete': 'off', 'data-status-dependent': 'true'},
            'foto_barang': {'accept': '.jpg,.jpeg,.png', 'class': 'input-file--proxy', 'data-inline-file-input': 'true', 'data-inline-file-placeholder': 'Pilih file', 'data-inline-file-extensions': 'jpg,jpeg,png', 'data-inline-file-error': 'Foto Barang hanya boleh berupa file JPG, JPEG, atau PNG.', 'data-status-dependent': 'true'},
            'catatan': {'placeholder': 'Tambahkan catatan bila diperlukan', 'rows': 4, 'data-status-dependent': 'true'},
        ...
        self.fields['volume'].widget.attrs.update({
            ...
            'class': (self.fields['volume'].widget.attrs.get('class', '') + ' is-readonly-field').strip(),
        ...
        self.fields['tanggal_pemeliharaan_info'].initial = getattr(self.instance, 'tanggal_pemeliharaan', None)
        self.fields['tanggal_perbaikan_info'].initial = getattr(self.instance, 'tanggal_perbaikan', None)
        if not getattr(self.instance, 'pk', None):
            self._set_required_state(['tahun_perolehan'])
        self.fields['tahun_perolehan'].error_messages.setdefault('invalid', 'Tahun perolehan harus berupa angka yang valid.')
    ...
    def _get_ketersediaan_value(self):
        ...
        kondisi = self.data.get('kondisi_barang') or self.initial.get('kondisi_barang') or 'Baik'
        return KetersediaanChoices.TERSEDIA if kondisi == 'Baik' else KetersediaanChoices.TIDAK_TERSEDIA
    ...
    def _queue_file_for_delete(self, file_obj):
        ...
        queued_names = {getattr(queued_file, 'name', '') for queued_file in self._files_to_delete_after_save}
    ...
    def clean(self):
        ...
        if getattr(self._meta.model, 'lock_volume_to_one', True):
            cleaned_data['volume'] = self._get_locked_volume_value(cleaned_data.get('kondisi_barang'))
        ...
        if status_barang == StatusBarangChoices.BMN and not kode_aset_bmn:
            self.add_error('kode_aset_bmn', 'Kode Aset BMN wajib diisi untuk barang berstatus BMN.')
...
class BarangLaboratoriumForm(AssetBaseForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['kategori_barang'].choices = [("", 'Pilih kategori barang'), *KategoriBarangLaboratoriumChoices.choices]
        self.fields['ik_alat'].widget = forms.FileInput(attrs={'accept': '.pdf,application/pdf', 'class': 'input-file--proxy', 'data-inline-file-input': 'true', 'data-inline-file-placeholder': 'Pilih file', 'data-inline-file-extensions': 'pdf', 'data-inline-file-error': 'IK Alat hanya boleh berupa file PDF.'})
...
class VolumeBaikAssetFormMixin:
    total_volume_info = forms.IntegerField(label='Total Volume', required=False, disabled=True, min_value=0)
    ...
    def _get_bmn_volume_values(self, kondisi_barang=None):
        kondisi = (kondisi_barang or self._get_kondisi_barang_value() or KondisiBarangChoices.BAIK).strip()
    ...
    def _force_bound_bmn_volume_data(self, volume_baik, volume_rusak):
        ...
        mutable_data[self.add_prefix('total_volume_info')] = str((volume_baik or 0) + (volume_rusak or 0))
    ...
    def _setup_volume_baik_fields(self):
        ...
        volume_field.error_messages.setdefault('invalid', 'Volume baik harus berupa angka yang valid.')
        ...
        volume_rusak_field.error_messages.setdefault('required', 'Volume rusak wajib diisi.')
        volume_rusak_field.error_messages.setdefault('invalid', 'Volume rusak harus berupa angka yang valid.')
        volume_rusak_field.error_messages.setdefault('min_value', 'Volume rusak minimal 0.')
        ...
        if is_non_bmn:
            ...
            for field in (volume_field, volume_rusak_field):
                ...
                if 'is-readonly-field' not in current_class.split():
                    field.widget.attrs['class'] = (current_class + ' is-readonly-field').strip()
        else:
            for field in (volume_field, volume_rusak_field):
                field.disabled = False
                field.widget.attrs.pop('disabled', None)
    ...
    def clean(self):
        cleaned_data = super().clean()
        status_barang = cleaned_data.get('status_barang') or self._get_status_barang_value()
    ...
    def save(self, commit=True):
        ...
        if instance.status_barang == StatusBarangChoices.BMN:
            volume_baik, volume_rusak = self._get_bmn_volume_values(instance.kondisi_barang)
...
class FasilitasRuanganForm(VolumeBaikAssetFormMixin, AssetBaseForm):
    ...
    class Meta(AssetBaseForm.Meta):
        ...
        fields = AssetBaseForm.Meta.fields + ['volume_rusak', 'total_volume_info', 'kategori_barang']
        labels = {
            'status_barang': 'Status Barang',
            'nama_barang': 'Nama Barang',
            'tipe_merek_barang': 'Tipe / Merek Barang',
...
class BarangPenunjangOperasionalForm(BaseMasterDataForm):
    ...
    class Meta:
        ...
        fields = ['nama_barang', 'tipe_merek_barang', 'volume', 'volume_rusak', 'satuan', 'kategori_barang']
        labels = {
            'nama_barang': 'Nama Barang',
            'tipe_merek_barang': 'Tipe / Merek Barang',
            'volume': 'Volume Baik',
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._set_input_attrs({
            ...
            'tipe_merek_barang': {'placeholder': 'Masukkan tipe / merek barang', 'autocomplete': 'off'},
            'volume': {'min': '0', 'data-volume-baik-field': 'true'},
            'volume_rusak': {'min': '0', 'data-volume-rusak-field': 'true'},
        })
        self.fields['total_volume_info'].widget.attrs['data-total-volume-field'] = 'true'
        self.fields['total_volume_info'].initial = getattr(self.instance, 'total_volume', 0)
        self.fields['ketersediaan_info'].initial = getattr(self.instance, 'ketersediaan', KetersediaanChoices.TERSEDIA)
        self.fields['volume'].error_messages.setdefault('invalid', 'Volume baik harus berupa angka yang valid.')
        self.fields['volume'].error_messages.setdefault('min_value', 'Volume baik minimal 0.')
        self.fields['volume_rusak'].error_messages.setdefault('invalid', 'Volume rusak harus berupa angka yang valid.')
        self.fields['volume_rusak'].error_messages.setdefault('min_value', 'Volume rusak minimal 0.')
...
class BahanOperasionalForm(BaseMasterDataForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._set_placeholder_choice('satuan', 'Pilih satuan')
        self._set_input_attrs({
            'nama_barang': {'placeholder': 'Masukkan nama bahan operasional', 'autocomplete': 'off'},
            'volume': {'min': '0'},
            'stok_minimum': {'min': '1'},
        })
        self.fields['ketersediaan_info'].initial = getattr(self.instance, 'ketersediaan', 'Habis')
        self.fields['volume'].error_messages.setdefault('invalid', 'Volume harus berupa angka yang valid.')
        self.fields['stok_minimum'].error_messages.setdefault('invalid', 'Stok minimum harus berupa angka yang valid.')
        self.fields['stok_minimum'].error_messages.setdefault('min_value', 'Stok minimum minimal 1.')
